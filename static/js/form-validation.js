	...
	return {
		...
		validateEmail: function (event) {
			...
			if (this.email && !emailRegex.test(this.email)) {
				...
				event.target.form.querySelector(
					'button[type="submit"]'
				).disabled = true;
			} else {
				this.emailError = '';
		...
		validateImage: function () {
			...
			if (this.imageFile) {
				if (!allowedExtensions.exec(this.imageFile.name)) {
					this.imageError =
						'Please choose a .jpg, .jpeg or .png image file';
					return false;
				} else if (this.imageFile.size > 10 * 1024 * 1024) {
					this.imageError =
						'Please choose an image file smaller than 10MB';
					return false;
				} else {
					this.imageError = '';
					return true;
		...
		validatePdf: function () {
			var allowedExtensions =
				/(\.pdf|\.doc|\.docx|\.pptx|\.xls|\.xlsx)$/i;
			if (this.pdfFile) {
				if (!allowedExtensions.exec(this.pdfFile.name)) {
					this.pdfError =
						'Please choose a valid file (.pdf, .doc, .docx, .pptx, .xls, .xlsx)';
					return false;
				} else if (this.pdfFile.size > 95 * 1024 * 1024) {
					this.pdfError =
						'Please choose a PDF file smaller than 95MB';
					return false;
				} else {
					this.pdfError = '';
					return true;
		...
		validateVideo: function () {
			...
			if (this.videoFile) {
				if (!allowedExtensions.exec(this.videoFile.name)) {
					this.videoError =
						'Please choose a .mov, .mp4, .avi, .flv or .wmv video file';
					return false;
				} else if (this.videoFile.size > 95 * 1024 * 1024) {
					this.videoError =
						'Please choose a video file smaller than 95MB';
					return false;
				} else {
					this.videoError = '';
					return true;
		...
		checkAllFieldsFilled: function (form) {
			...
			let allFilled = Array.from(requiredInputs).every(
				(input) => input.value !== ''
			);
...
function customFileUpload() {
	...
	$('.image-file-input').on('change', function (event) {
		...
		$(this)
			.closest('.file-upload__container')
			.find('[data-invalid-feedback]')
			.text(validator.imageError);
	});
